...
- **Update check:** fetches this fork's latest GitHub release (`https://api.github.com/repos/xhqing/pi/releases/latest`) to check whether a newer fork version exists. Disable it with `PI_SKIP_VERSION_CHECK=1`. Disabling update checks only turns off this check.
...
| `PI_SKIP_VERSION_CHECK` | Skip the Pi version update check at startup. This prevents the fork GitHub latest-release request |
