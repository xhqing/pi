...
| `cursorStyle` | string | `"block"` | Editor caret style: `"block"` draws the reverse-video block caret; `"hardware"` hides it and uses the terminal's native cursor as the caret (implies `showHardwareCursor`) |
